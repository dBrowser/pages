module.exports = class FileTree {
  ...
  async setup () {
    // list all files
    let entries = await this.vault.readdir('/', {stat: true, recursive: !this.opts.onDemand})
  ...
  async readFolder (node) {
    // list all files
    let entries = await this.vault.readdir(node.entry.name, {stat: true})
